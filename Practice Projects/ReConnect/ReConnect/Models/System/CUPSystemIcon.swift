//
//  CUPSystemIcon.swift
//  ReConnect
//
//  Created by Harry Dinh on 2023-05-27.
//

import Foundation

class CUPSystemIcon {
    public static let edit = "pencil"
    
    public static let share = "square.and.arrow.up"
    
    public static let exit = "rectangle.portrait.and.arrow.right"
    
    public static let delete = "trash"
    
    public static let compose = "square.and.pencil"
    
    public static let profile = "person.crop.circle"
    
    public static let newProfile = "person.crop.circle.badge.plus"
    
    public static let qrcode = "qrcode"
    
    public static let folder = "folder"
    
    public static let newFolder = "folder.badge.plus"
    
    public static let message = "message"
    
    public static let settings = "gear"
    
    public static let emailEnvelope = "envelope"
    
    public static let passwordLock = "lock"
    
    public static let gift = "gift"
    
    public static let person = "person"
    
    public static let add = "plus"
    
    public static let infoCircle = "info.circle"
    
    public static let newspaper = "newspaper"
    
    public static let magnifyingglass = "magnifyingglass"
    
    public static let menuList = "list.bullet"
    
    public static let filter = "line.3.horizontal.decrease.circle"
    
    public static let notification = "bell"
    
    public static let camera = "camera"
    
    public static let photo = "photo"
}
